...
void ZombieEnem::ChooseEnemy() {
	if (app->BTSystem->Zombiebattle == true) {	
		...
		for (int i = 1; i < Zbie[WhichZombie].numEnemies + 1; i++) {
			...
			if (Zbie[i].dead == false && x > 1005 && x < 1110 && y > -5 + 120 * i && y < 115 + 120 * i + 100 && (app->input->GetMouseButtonDown(1) == KEY_DOWN || app->input->GetKey(SDL_SCANCODE_RETURN) == KEY_DOWN || app->input->Pad->GetButton(SDL_CONTROLLER_BUTTON_A) == KEY_DOWN) && app->BTSystem->AttackPlayer != 0 && app->BTSystem->PlayerTurn == true && app->BTSystem->SpecialAttackEnable == false) {
				app->BTSystem->ZombieTarget = i;
				//SDL_Rect Enem1 = { app->player->P1.position.x + 500, app->player->P1.position.y - 330 + 120 * i, 100, 100 };
			}
		}
		for (int i = 1; i < Zbie[WhichZombie].numEnemies + 1; i++) {//
			if (Zbie[i].dead == false && x > 1005 && x < 1110 && y > -5 + 120 * i && y < 115 + 120 * i + 100 && (app->input->GetMouseButtonDown(1) == KEY_DOWN || app->input->GetKey(SDL_SCANCODE_RETURN) == KEY_DOWN || app->input->Pad->GetButton(SDL_CONTROLLER_BUTTON_A) == KEY_DOWN) && app->BTSystem->PlayerTurn == true && app->BTSystem->SpecialAttackEnable == false && app->input->GetKey(SDL_SCANCODE_LCTRL) == KEY_REPEAT && app->player->godMode == true) {
				//app->BTSystem->ZombieTarget = i;
				Zbie[i].hp = 0;
			}
